...
const App: React.FC = () => {
      ...
      interval = window.setInterval(() => {
        setRunState(prev => {
          ...
          if (manualHR) {
            ...
            const targetHR = 100 + (prev.currentSpeed * 10);
  ...
  const handleSyncHealth = () => {
    setIsSyncing(true);
    // Simulate Apple Health Kit Interaction
    setTimeout(() => {
      setIsSyncing(false);
      alert("Success! Your activity has been synced to Apple Health.");
    }, 1500);
  };
  ...
  const handleUnitChange = (u: 'imperial' | 'metric') => {
    ...
    if (u === 'imperial') {
       setWeightInput(Math.round(weightInput / LBS_TO_KG));
    } else {
       setWeightInput(Math.round(weightInput * LBS_TO_KG));
    }
  };
      ...
      <div className="flex-1 flex flex-col justify-start space-y-6">
        <div className="text-center mt-6">
          ...
          <div className="flex items-center justify-center gap-2 mt-2">
            <p className="text-slate-400">Professional Performance Tracker</p>
        ...
        <div className="bg-slate-800 p-6 rounded-2xl border border-slate-700 shadow-xl space-y-6">
          ...
          <div className="pt-2 border-t border-slate-700">
             <label className="text-xs uppercase font-bold text-slate-500 mb-2 block">Devices & Health Integration</label>
             <div className="space-y-2">
                ...
                >
                   <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-slate-300">Wearable Heart Monitor</span>
                   </div>
                   <span className={`text-xs px-2 py-1 rounded ${settings.devices.fitbitConnected ? 'bg-teal-500 text-slate-900 font-bold' : 'text-slate-500'}`}>
                     {settings.devices.fitbitConnected ? 'READY' : 'OFF'}
                   </span>
                </button>
                ...
                >
                   <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm font-medium text-slate-300">Glucose (CGM) Sync</span>
                   </div>
                   <span className={`text-xs px-2 py-1 rounded ${settings.devices.glucoseMonitorConnected ? 'bg-blue-500 text-white font-bold' : 'text-slate-500'}`}>
                     {settings.devices.glucoseMonitorConnected ? 'READY' : 'OFF'}
                   </span>
                </button>
             </div>
          </div>
  ...
  const renderRunning = () => (
    <div className="flex flex-col h-screen bg-slate-900 overflow-hidden relative">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 pb-40">
        ...
        {/* Speedometer Card Grid */}
        <div className="grid grid-cols-3 gap-2">
          <div className="bg-slate-800 p-3 rounded-2xl border border-slate-700 flex flex-col items-center justify-center">
             <span className="text-[10px] text-slate-400 uppercase font-bold text-center">Current Pace</span>
             <span className="text-xl font-bold text-teal-400">{displayPace}</span>
             <span className="text-[10px] text-slate-500">min/{settings.unit === 'imperial' ? 'mi' : 'km'}</span>
          </div>
          <div className="bg-slate-800 p-3 rounded-2xl border border-slate-700 flex flex-col items-center justify-center">
             <span className="text-[10px] text-slate-400 uppercase font-bold text-center">Speed</span>
             <span className="text-xl font-bold text-yellow-400">{displaySpeed}</span>
             <span className="text-[10px] text-slate-500">{settings.unit === 'imperial' ? 'MPH' : 'KPH'}</span>
          </div>
           <div className="bg-slate-800 p-3 rounded-2xl border border-slate-700 flex flex-col items-center justify-center">
             <span className="text-[10px] text-slate-400 uppercase font-bold text-center">Arrival</span>
             <span className="text-lg font-bold text-blue-400 leading-tight">{estimatedArrival}</span>
             <span className="text-[9px] text-slate-500">Finish: {estimatedDuration}</span>
          </div>
        </div>
        ...
        <div className="bg-slate-800 p-4 rounded-2xl border border-slate-700">
           <div className="flex items-center justify-between mb-3">
              <h3 className="text-slate-200 font-bold text-sm uppercase">Bio-Metrics</h3>
              {settings.devices.fitbitConnected && <span className="text-[10px] text-teal-500 bg-teal-900/30 px-2 py-0.5 rounded">Syncing HR</span>}
           </div>
           
           <div className="grid grid-cols-3 gap-2">
              <div className="bg-slate-900/50 p-3 rounded-xl flex flex-col items-center justify-center relative">
                 <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-4 h-4 text-red-500 animate-pulse absolute top-2 right-2 opacity-80">
                 ...
                 {!settings.devices.fitbitConnected && (
                    <input 
                      type="number" 
                      placeholder="Set" 
                      className="mt-1 w-full text-center bg-slate-800 text-[10px] text-slate-300 rounded border-none p-1 focus:ring-1 focus:ring-teal-500"
                      value={manualHR}
      ...
      {/* FIXED Floating Controls - Always visible */}
      <div className="fixed bottom-8 left-0 right-0 px-6 flex justify-center gap-4 z-[1000]">
        <button 
          onClick={() => setRunState(prev => ({ ...prev, isPaused: !prev.isPaused }))}
          className={`h-20 w-20 rounded-full flex items-center justify-center shadow-2xl transition-all active:scale-90 ${runState.isPaused ? 'bg-teal-500 text-white' : 'bg-yellow-500 text-slate-900'}`}
        >
           {runState.isPaused ? (
             <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-10 h-10 ml-1"><path strokeLinecap="round" strokeLinejoin="round" d="M5.25 5.653c0-.856.917-1.398 1.667-.986l11.54 6.348a1.125 1.125 0 0 1 0 1.971l-11.54 6.347a1.125 1.125 0 0 1-1.667-.985V5.653Z" /></svg>
           ) : (
             <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-10 h-10"><path strokeLinecap="round" strokeLinejoin="round" d="M15.75 5.25v13.5m-7.5-13.5v13.5" /></svg>
           )}
        </button>
        ...
        {runState.isPaused && (
          <button 
            onClick={handleStop}
            className="h-20 w-20 bg-red-500 rounded-full flex items-center justify-center shadow-2xl text-white hover:bg-red-600 transition-all active:scale-90 animate-fade-in"
          >
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={2.5} stroke="currentColor" className="w-10 h-10"><path strokeLinecap="round" strokeLinejoin="round" d="M5.25 7.5A2.25 2.25 0 0 1 7.5 5.25h9a2.25 2.25 0 0 1 2.25 2.25v9a2.25 2.25 0 0 1-2.25 2.25h-9a2.25 2.25 0 0 1-2.25-2.25v-9Z" /></svg>
          </button>
        )}
      </div>
    </div>
  ...
  const renderSummary = () => (
    <div className="h-full p-6 flex flex-col justify-center max-w-md mx-auto py-12">
      <div className="bg-slate-800 rounded-3xl p-8 border border-slate-700 shadow-2xl text-center">
        ...
        <p className="text-slate-400 text-sm mb-8">{new Date().toLocaleDateString('en-US', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
        
        <div className="grid grid-cols-2 gap-6 mb-8">
           <div className="bg-slate-900/40 p-4 rounded-2xl">
             <p className="text-xs uppercase text-slate-500 font-bold mb-1">Distance</p>
             <p className="text-2xl font-mono text-white">{displayDistance}</p>
             <p className="text-[10px] text-slate-600 font-bold tracking-widest">{settings.unit === 'imperial' ? 'MILES' : 'KM'}</p>
           </div>
           <div className="bg-slate-900/40 p-4 rounded-2xl">
             <p className="text-xs uppercase text-slate-500 font-bold mb-1">Time</p>
             <p className="text-2xl font-mono text-white">{formatDuration(runState.elapsedTime)}</p>
             <p className="text-[10px] text-slate-600 font-bold tracking-widest">DURATION</p>
           </div>
        </div>

        <div className="border-t border-slate-700 pt-8 grid grid-cols-2 gap-6 mb-8">
           <div>
             <p className="text-xs uppercase text-slate-500 font-bold">Avg Pace</p>
             <p className="text-xl font-mono text-white">
                {calculatePace(runState.totalDistance > 0 ? runState.totalDistance / (runState.elapsedTime || 1) : 0, settings.unit)}
             </p>
           </div>
           <div>
        ...
        <div className="space-y-3 mb-8">
          <button 
            ...
            disabled={isSyncing}
            className={`w-full py-4 rounded-2xl font-bold flex items-center justify-center gap-3 transition-all ${isSyncing ? 'bg-slate-700 text-slate-400 cursor-wait' : 'bg-slate-100 text-slate-900 hover:bg-white active:scale-95'}`}
          >
            {isSyncing ? (
            ...
            {isSyncing ? 'Syncing...' : 'Sync to Apple Health'}
          </button>
          
          {isSaving && <p className="text-xs text-teal-400 animate-pulse">Backing up run data...</p>}
        </div>

        <button 
          onClick={() => setView(AppView.SETUP)}
          className="w-full py-4 bg-slate-700/50 hover:bg-slate-700 text-slate-300 rounded-2xl font-semibold transition-all"
        >
          Return Home
        </button>
      </div>
